...
```{r eval = FALSE}
library(tidyverse)
my_data <- read_csv("~/Desktop/documents/Github/portfolio-gabedonhig-2/data/mydata.csv")
```
...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Documents/GitHub/portfolio-gabedonhig-2/data/imdb_5000_messy.csv")
```
...
-   What type of info is provided on quantitative variables?
  - We can see stuff like the mean, median, minimum and maximums of quantitative variables
-   What type of info is provided on categorical variables?
  - We can see total observations of Categorical variables in descending order
-   What stands out to you in these summaries? Is there anything you'd need to clean before using this data?
  - I can see in the genres section that many movies are listed as comedy or drama, and I would like to know if they are listed three times as comedy or drama, then comedy, then drama, or if they only appear as one observation that is comedy and drama. 
...
-   What's goofy about this / what do we need to fix?
  - There are 3 different values that all mean color, just with different cases. There is also 2 values representing black and white, with different spellings
-   More specifically, what different categories does the `color` variable take, and how many movies fall into each of these categories?
  - There is color(4815), black and white(209), and NA(19)

```{r}
imdb_messy |>
  mutate(color = str_replace(color, "(COLOR|color)", "Color"))
```
...
```{r}
imdb_messy |>
  mutate(color = fct_recode(color, "Color" = "Color", "Color" = "COLOR", "Color" = "color", "Black_White" = "B&W", "Black_White" = "Black and White"))
```
...
imdb_messy |>
  mutate(color = str_replace(color, "(COLOR|color)", "Color")) |>
  mutate(color = str_replace(color, "(B&W|Black and White)", "Black_White"))
```
...
- It is probable that smaller budget films, that have less attention, accountants, and assistants to gather data would have less data.
...
- The movies that seem to have more NA values in actor_1_facebook_likes seem to be documentaries, have fewer faces in their posters and have few critic reviews. This makes sense, as documentaries are not meant to use actors, the fewer faces in the poster means that actors in the movie would get less attention, and fewer critic reviews means less attention towards the movie
...
-   What did `drop_na()` do? How many data points are left?
  - drop_na() removed all observations with a NA value. There are still 3756 points left
-   In what situations might this function be a good idea?
  - If you want to create a dataset with absolutely no NA values, this is a good idea
-   In what situations might this function be a bad idea?
  - If you want a dataset with as many observations as possible, even if some are missing some values, then don't use this
