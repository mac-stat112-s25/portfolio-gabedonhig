...
# Units of observation = groups of penguins grouped by sex and species
head(penguin_avg)
```

2.  Did the reshaping process result in any information loss from the original data?
  - yes it did. Now the data does not include individual penguins, but instead groups of penguins grouped by sex and species
...
```{r}
ggplot(sleep_long, aes(x = day, y = reaction_time, color = Subject, alpha = 0.5)) +
  geom_point()
```
